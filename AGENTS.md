...
The prose scan is a Node program, `hooks/scan-prose.ts`, run through
`adg scan-prose`. It used to be a bash script needing **bash 4 or
newer**, which meant it refused to run at all under the bash macOS
ships; that script is gone. Node is already a hard requirement of this
package, so the scan now adds no second one, and `adg check` runs all
eight of its checks on a fresh Mac.
